...
class AnnotationTable(tag: Tag) extends Table[Annotation] (tag, Some("cnrs"), "simulation_city_annotation") {
  ...
  def pk = primaryKey("pk_simulation_city_annotation", (simulationId, trafficLevel, startTs, endTs))
  ...
  def indexAnnotation = index("idx_simulation_city_annotation", description)
}
...
object AnnotationData extends TableQuery[AnnotationTable](new AnnotationTable(_)) {
  ...
  def createSchema() =
    if (blockingExec {
      MTable.getTables("simulation_city_annotation")
