...
import org.speedd.ml.loaders.{DataLoader, cnrs, fz}
...
object DataLoaderApp extends CLIDataLoaderApp {
  ...
  // -------------------------------------------------------------------------------------------------------------------
  opt("t", "task", "Specify one of the following tasks: fz.input, cnrs.collected.input, cnrs.collected.annotation," +
  ...
  val loader: DataLoader = taskOpt.getOrElse(fatal("Please specify a task")) match {
    ...
    case "fz.input" => fz.InputDataLoader
    case name =>
