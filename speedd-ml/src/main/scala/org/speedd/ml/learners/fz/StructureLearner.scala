package org.speedd.ml.learners.fz
...
import org.speedd.ml.loaders.fz.TrainingBatchLoader
...
final class StructureLearner private(kb: KB,
  ...
  private lazy val learner = OSLa(kb, kbConstants, nonEvidenceAtoms, targetSignatures, modes, maxLength,
    allowFreeVariables = false, threshold, theta, lambda = lambda, eta = eta)
  ...
  override def trainFor(startTs: Int, endTs: Int, batchSize: Int,
    ...
    for ( ((currStartTime, currEndTime), idx) <- microIntervals.zipWithIndex) {
      ...
      val batch = batchLoader.forIntervalSL(currStartTime, currEndTime, None, useOnlyConstants)
...
object StructureLearner extends Logging {
  ...
  def apply(inputKB: File,
    ...
    new StructureLearner(
      kb, kbConstants, kb.predicateSchema, sqlAtomMappings, inputKB,
